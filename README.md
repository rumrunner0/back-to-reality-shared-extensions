...
General-purpose toolkit of utilities and extensions for everyday .NET code. Zero external dependencies (BCL only), nullable-annotated, strong-named, covered by tests. Targets .NET 9.
...
| Area | Highlights |
| --- | --- |
| `Collections` | Bounded counting predicates (`Some`, `None`, `Many`, `Exactly`, `AtLeast`, `MoreThan`, `LessThan`) that never enumerate further than needed — safe on infinite sequences; inclusive range removal for lists; tuple deconstruction of lists (`Deconstruct2`–`Deconstruct8`); collection-initializer `Add` for any `ICollection<T>`; reference-equality hash sets; infinite `Cycle` over a collection. |
...
| `Disposing` | `DisposableGroup` — treat many disposables as one: reverse-order disposal, thread-safe single shot, failures collected instead of thrown. |
| `Environment` | Required environment variables (`GetRequired`, `GetRequiredInt`) with culture-invariant parsing. |
| `Exceptions` | Guard helpers (`ThrowIfNull`, `ThrowIfNullOrEmpty`, `ThrowIfAnyNull`, …) that capture the argument expression and teach the compiler null-state via `[NotNull]`; `AggregateException`-aware inner-exception search (`IsOrHasInner`, `HasInner`) and message joining (`JoinMessages`). |
| `Extensions` | String emptiness checks, JSON validation, Base64 decoding, GUID parsing, whitespace splitting, a pragmatic email regex, and fluent object shaping (`Shape`, `Chain`, `Follow`). |
...
| `Time` | `DateTimeOffset.Truncate(resolution)` and `TruncatedTimeProvider` — a `TimeProvider` truncated to microseconds for storage-friendly timestamps. |
...
- No external NuGet dependencies — the package brings in nothing but itself.
- Nullable reference types are first-class: guards carry `[NotNull]`, predicates carry `[NotNullWhen]`, so flow analysis works with the library, not against it.
- The build policy is zero warnings, and every public member is XML-documented.
